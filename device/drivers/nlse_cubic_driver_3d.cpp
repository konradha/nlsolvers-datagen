...
int run(const driver::Complex3DArgs &args) {
  ...
  const Eigen::SparseMatrix<std::complex<double>> L =
      (build_anisotropic_laplacian_noflux_3d<std::complex<double>>(
           grid.nx - 2, grid.ny - 2, grid.nz - 2, grid.dx, grid.dy, grid.dz,
           c_complex))
          .eval();
  ...
  device::NLSESolverDevice::Parameters params(args.num_snapshots,
                                              snapshot_freq, 25);
  device::NLSESolverDevice solver(L, u0.data(), m.data(), true, params);
...
int main(int argc, char **argv) {
  if (argc != 14) {
    std::cerr << "Usage: " << argv[0] << driver::kComplex3DUsage << '\n';
    return 1;
  }

  try {
    return run(driver::parse_complex_3d_args(argv));

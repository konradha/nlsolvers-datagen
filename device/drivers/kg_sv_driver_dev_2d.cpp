...
int run(const driver::Wave2DArgs &args) {
  ...
  Eigen::VectorXd v0 = driver::read_npy_checked<double>(
      args.input_v0, {grid.ny, grid.nx}, "v0");
  ...
  const Eigen::SparseMatrix<double> L =
      (build_anisotropic_laplacian_noflux<double>(grid.nx - 2, grid.ny - 2,
                                                   grid.dx, grid.dy, c))
          .eval();
  ...
  device::KGESolverDevice::Parameters params(args.num_snapshots, snapshot_freq,
                                             10);
  ...
  driver::run_kge_steps(solver, args.nt, snapshot_freq, args.num_snapshots,
                        [](device::KGESolverDevice &solver) { solver.step_sv(); });
...
int main(int argc, char **argv) {
  if (argc != 14) {
    std::cerr << "Usage: " << argv[0] << driver::kWave2DUsage << '\n';
    return 1;
  }

  try {
    return run(driver::parse_wave_2d_args(argv));
